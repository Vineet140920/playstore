...
const OwnerAuth = ({ onLogin }) => {
    ...
    const handleAuth = async () => {
        const url = isLogin 
            ? 'http://localhost:8081/api/owners/login' 
            : 'http://localhost:8081/api/owners/register';
        ...
        try {
            ...
            alert(`Owner ${isLogin ? 'logged in' : 'registered'} successfully!`);
            if (isLogin && onLogin) {
    ...
    return (
        <div style={styles.container}>
            <h2>{isLogin ? 'Owner Login' : 'Owner Register'}</h2>
            <input
                type="text"
